...
// which are part of the ZEVNO repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="zevno-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /zevno-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "zevnopay-zevno/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from zevnopay-zevno/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'zevnopay-zevno/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from zevnopay-zevno/develop"
            }
          }

          if (hasCache) {
            sh "cd /zevno-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /zevno-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /zevno-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /zevno-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /zevno-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /zevno-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /zevno-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
