UNIX BUILD NOTES
====================
Some notes on how to build ZEVNO Core in Unix.

(for OpenBSD specific instructions, see [build-openbsd.md](build-openbsd.md))

Base build dependencies
-----------------------
Building the dependencies and ZEVNO Core requires some essential build tools and libraries to be installed before.
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, ZEVNO Core should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, ZEVNO Core may be compiled in
...
Add `CC=egcc CXX=eg++ CPP=ecpp` to the dependencies build and the ZEVNO Core build:
```bash
$ cd depends
