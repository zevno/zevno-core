...
# zevno_hash
RUN git clone https://github.com/zevnopay/zevno_hash
RUN cd zevno_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} zevno
RUN useradd -u ${USER_ID} -g zevno -s /bin/bash -m -d /zevno zevno
...
RUN mkdir /zevno-src && \
  ...
  chown $USER_ID:$GROUP_ID /zevno-src && \
