...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 ZEV irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on zevnocentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in zevno.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("zevno.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in zevno.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* ZEVNO features */
    { "zevno",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "zevno",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "zevno",               "gobject",                &gobject,                true,  {} },
    { "zevno",               "voteraw",                &voteraw,                true,  {} },
