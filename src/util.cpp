...
const char * const BITCOIN_CONF_FILENAME = "zevno.conf";
const char * const BITCOIN_PID_FILENAME = "zevnod.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "zevno" is a composite category enabling all ZEVNO-related debug output
                if(ptrCategory->count(std::string("zevno"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZEVNO
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZEVNO
    // Mac: ~/Library/Application Support/ZEVNO
    // Unix: ~/.zevno
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZEVNO";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZEVNO";
#else
    // Unix
    return pathRet / ".zevno";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty zevno.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override zevno.conf
